...
function dayOfQuarter( month, day, year ) {
	...
	if ( arguments.length === 0 ) {
		...
		if ( isDateObject( month ) ) {
			...
			throw new TypeError( format( '1Q7AW,La', month ) );
		}
	} else {
		if ( !isString( month ) && !isInteger( month ) ) {
			throw new TypeError( format( '1Q7AX,LV', month ) );
		}
		if ( !isInteger( day ) ) {
			throw new TypeError( format( '1Q77f,LW', day ) );
		}
		if ( !isInteger( year ) ) {
			throw new TypeError( format( '1Q72z,GL', year ) );
		}
